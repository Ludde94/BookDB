...
export const exportDataToXML = async () => {
  ...
  const allBooksData = JSON.stringify(
    { libraryBooks, wantToReadBooks },
    null,
    2
  );
  return allBooksData;
...
export const moveBookToLibrary = async (bookId) => {
  try {
    ...
    if (book) {
      ...
      console.log(
        `Book with ID ${bookId} moved from "want to read" list to library.`
      );
    } else {
...
export const moveBookToWantToRead = async (bookId) => {
  try {
    ...
    if (book) {
      ...
      console.log(
        `Book with ID ${bookId} moved from library to "want to read" list.`
      );
    } else {
